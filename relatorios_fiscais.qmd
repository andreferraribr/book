...
previ <- previ %>% mutate (receita =rowSums( previ[,startsWith(names(previ),"movimento_r")]), despesa  =rowSums( previ[,startsWith(names(previ),"movimento_d")]), resultado = receita - despesa)
...
previ_clean <- previ  %>% select("mes",starts_with("Movimento"), "resultado")


# previ_clean$mes <-  ceiling_date(previ_clean$mes,"month")+ months(1)-days(1)

datatable(previ_clean)%>% formatCurrency(2:10,
  currency = "R$ ",
  interval = 3,
  mark = ".",
  digits = 2,
...
# 
# previ_plot <- previ_clean %>% select(!acumulado) %>% pivot_longer(!mes,names_to = "item", values_to = "saldo_acumulado") %>% mutate(saldo_acumulado = if_else(startsWith(item,"Despesas"), saldo_acumulado*-1,saldo_acumulado))

previ_plot <- previ_clean  %>% pivot_longer(!mes,names_to = "item", values_to = "saldo_acumulado") %>% mutate(saldo_acumulado = if_else(startsWith(item,"movimento _despesa"), saldo_acumulado*-1,saldo_acumulado))


# ggplot(previ_plot %>% filter(item != "resultado") )+geom_area(  aes(x= mes, y = saldo_acumulado, fill= item))+geom_line(previ_plot %>% filter(item == "resultado"), aes(x= mes, y = saldo_acumulado))

p <- ggplot(NULL,aes(x= mes, y = saldo_acumulado) )+
  geom_area(data = previ_plot %>% filter(item != "resultado"), aes(fill = item))+
  geom_line(data =previ_plot %>% filter(item == "resultado"), color= "red" )
p
