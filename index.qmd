...
2.  apresentando a série histórica suavizada (média dos últimos doze meses) do saldo de restos a pagar a pagar:
...
Para facilitar os cálculos, vamos criar variável **saldo** a partir da soma dos saldos de RP Procesado a Pagar e RP *Não* Processado a Pagar.
...
Apresentamos na Tabela 1 o saldo de Restos a Pagar a Pagar detalhado por ano do empenho e ano do lançamento.
...
datatable(rp_ano %>% group_by(ano_empenho, ano_lancamento) %>% summarise(saldo = sum(saldo)) %>% filter(saldo !=0), options = list(
  ...
  caption = 'Tabela 1. Restos a Pagar a Pagar: saldo por ano do empenho.')%>% formatCurrency(3, 'R$ ', mark = ".",
  digits = 2,  dec.mark = ",")
```
...
datatable(rp_ano %>% group_by(tempo, ano_lancamento) %>% summarise(saldo = sum(saldo)) %>% arrange((ano_lancamento),desc(tempo)) %>% filter(saldo !=0), options = list(
  language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese.json'),
  pageLength = 10
),  caption = 'Restos a Pagar a Pagar: saldo por ano do empenho.')%>% formatCurrency(3, 'R$ ', mark = ".",
  digits = 2,  dec.mark = ",")
...
Criei a variável "defasagem" para identificar o tempo entre a emissão do empenho e o ano do lançamento do resto a pagar. Se considerarmos o ano fiscal de 2021, um empenho emitido em 2019 terá uma defasagem de 2 anos.
...
## Restos a Pagar: defasagem temporal do empenho
...
## Restos a Pagar *NÃO* Processados: defasagem temporal do empenho
...
## Restos a Pagar Processados: defasagem temporal do empenho
...
## Restos a Pagar Processados: variação entre 2019 e 2202
...
```{r}
datatable(rp %>% filter(ano == 2008) %>% select(data, rp) %>% adorn_totals("row"), options = list(
  language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese.json'),
  pageLength = 15
),
  caption = 'Restos a Pagar Processados a Pagar: maiores reduções entre os exercícios de 2019 e 2020 em R$ bilhões.')%>% formatCurrency(2, 'R$ ', digits = 1)
```
